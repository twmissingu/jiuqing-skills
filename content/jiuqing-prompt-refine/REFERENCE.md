...
打磨 prompt 时会问用户比例、时长、分镜等维度，但对应的生成 skill 不一定都能兑现。对齐前先把下面这些不可控项告知用户，别采集兑现不了的参数、别承诺做不到的效果。具体能力以对应生成 skill 的 REFERENCE 为准，这里只列最常见的落差。

**图片生成：**

- **比例需转成像素尺寸** —— 用户可能说"16:9 / 9:16"，但生成器接收的是 `宽x高` 像素（如 `1024x1024`），不直接收比例。对齐时把比例换算成具体像素再交付，常见对应：1:1→`1024x1024`、16:9→`1024x576`、9:16→`576x1024`、4:3→`1024x768`。
- **无跨图主体一致性** —— 同一角色/物体在多张图里保持一致，接口做不到。涉及"系列图、同一人物多张"时如实告知，别承诺。

**视频生成：**

- **时长固定，不可控** —— 成片固定 5 秒，传时长参数会被忽略。用户提"做个 30 秒""分三段各 10 秒"时，先说明只能出 5 秒，再据此压缩内容。
- **比例/分辨率固定** —— 固定 1280×704，比例不可选。别采集视频比例需求。
- **无参考图/跨段角色一致性** —— 不支持图生视频，也无法让同一角色稳定出现在多段里。分镜/多镜头叙事可以写进单条 prompt，但 5 秒内成片，且不保证主体连贯——涉及一致性时如实告知。

> 时间码与多段落写法（上方"时间码写法"）仍可用于在 5 秒内组织镜头推进，但不要据此让用户以为能产出更长或多镜头严格连贯的成片。
