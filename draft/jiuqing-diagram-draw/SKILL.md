...
- **字体**：`font-family: system-ui, -apple-system, 'Noto Sans SC', sans-serif`；标题 16px 加粗，正文 13-14px。
...
- 节点超过 **15 个**（经验值：>15 节点的 SVG 可读性急剧下降，人眼短期记忆上限约 7±2 个独立元素）→ 建议用户拆分为多个子图或聚合为容器节点。
...
| SVG 过长（>2000 字符，约 50 行 SVG 代码——超过此长度 agent 输出易截断或上下文溢出） | 分段输出，先给核心部分，再补细节 |
| 用户要求动画/交互式 SVG | 拒绝并说明："本 skill 生成静态 SVG，动画效果请使用 CSS animation 或 SMIL 在 SVG 外层包装" |
| 节点标签含非 ASCII 字符（中日韩） | 字体回退链需包含 CJK 字体：`font-family: system-ui, -apple-system, 'Noto Sans SC', sans-serif` |
