...
| 场景 | 信号 | 处理 |
|------|------|------|
| LICENSE 不存在 | `ls LICENSE` 无输出 | 告诉用户需要先选择许可证再继续，不跳过法律阶段 |
| 测试/构建失败 | 命令非零退出 | 如实报告失败点，不粉饰不跳过；用户决定是否修复后再继续 |
| CHANGELOG 格式错误 | 格式不合 keepachangelog 规范 | 帮用户修正格式，重新生成 |
| 版本文件不存在 | package.json/pyproject.toml 等缺失 | 告诉用户手动指定版本号，或先初始化项目结构 |
