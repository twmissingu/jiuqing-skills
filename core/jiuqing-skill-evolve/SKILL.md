...
3. **baseline score** —— 派 ≥3 个 independent judge（judge ≠ 后续 edit agent），各按 `evaluation-rubric.md` 在 train 与 holdout 上 score，每个 dimension 取 median。记录 baseline 分与 judge 间 variance。
...
4. **re-score** —— 派**新一批** independent judge（不复用上轮 judge，避免 anchoring 上轮分数），train 上 score 定位效果，holdout 上 score 作 keep 依据，每个 dimension 取 median。
5. **regression guard** —— 对比本轮与上轮**全部 dimension** 的 median。任一**非目标 cluster** 的 dimension 跌幅 > 2 分，即判为损坏他处，即便总分上升也 revert。
6. **keep / revert** —— keep 须同时满足：holdout 总 median 高于上轮、未触发 regression guard、judge variance 未显著放大。否则 `git revert`（**禁用 `git reset --hard`**，保留失败轨迹供分析）。baseline 只锁定被 keep 过的最高分，revert 轮不污染 baseline。
